...
class DatabaseRuleStoreAdapter implements RuleStoreAdapter
{
     ...
     * `firewall` database table. Any IP returned by Firewall::all() that is
     * NOT in this set was expanded from config arrays (CIDR, country:, host:,
     * file:...) by upstream's IpList::getNonDatabaseIps() and must be treated
     * as config-sourced and read-only, because Firewall::remove() cannot
     * mutate config-derived entries.
     */
